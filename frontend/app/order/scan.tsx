...
export default function ScanScreen() {
  ...
  const processImage = async (base64: string) => {
    ...
    try {
      const res = await fetch(`${BACKEND_URL}/api/scan`, {
        ...
        body: JSON.stringify({ image: base64, type: scanType }),
      });
  ...
  const scanTypes = [
    { key: 'receipt' as const, label: 'Receipt / Bill', icon: FileText, desc: 'Photo of a grocery bill' },
    { key: 'list' as const, label: 'Shopping List', icon: ShoppingBag, desc: 'Handwritten or printed list' },
    ...
    <View className="flex-1 bg-background">
      ...
      <ScrollView className="flex-1" contentContainerStyle={{ padding: 20 }}>
        {processing ? (
          ...
          <>
            {/* Scan Type Selection */}
            <Text className="mb-3 text-sm font-semibold text-foreground">What are you scanning?</Text>
